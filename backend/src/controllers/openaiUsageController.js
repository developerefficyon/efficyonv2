/**
 * OpenAI Cost Analysis Controller
 * Handles connect / disconnect / sync / usage queries for the OpenAI tool integration.
 *
 * Note: file is named openaiUsageController to avoid colliding with the existing
 * openaiService used for chat. This controller is purely for the cost-analysis tool.
 */
...
async function connect(req, res) {
  const endpoint = "POST /api/integrations/openai/connect"
  try {
    const user = req.user
    ...
    // Kick off an initial 90-day backfill in the background.
...
async function disconnect(req, res) {
  const endpoint = "DELETE /api/integrations/openai"
  try {
    const user = req.user
    if (!user) return res.status(401).json({ error: "Unauthorized" })
...
/**
 * POST /api/integrations/openai/sync
 * Manual on-demand sync (last 7 days).
 */
async function sync(req, res) {
  const endpoint = "POST /api/integrations/openai/sync"
  try {
    const user = req.user
    if (!user) return res.status(401).json({ error: "Unauthorized" })
    ...
    const { data: integration, error } = await supabase
      ...
      .maybeSingle()
    if (error) throw new Error(error.message)
    if (!integration) return res.status(404).json({ error: "OpenAI integration not configured" })
...
/**
 * GET /api/integrations/openai/usage?days=30
 * Returns daily series + model breakdown + summary metrics.
 */
async function getUsage(req, res) {
  const endpoint = "GET /api/integrations/openai/usage"
  try {
    const user = req.user
    if (!user) return res.status(401).json({ error: "Unauthorized" })
